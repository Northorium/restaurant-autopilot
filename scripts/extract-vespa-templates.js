// Extract REAL reply templates from Vespa reviews
const fs = require('fs');
const path = require('path');

const reviewsPath = path.join(__dirname, '../clients/Vespa-Humla/ALL-GOOGLE-REVIEWS.json');
...
withReplies.forEach(r => {
  ...
  // Check if English
  if (reply.match(/thank you|thanks|love|great|back here/i)) {
...
// Save templates
fs.writeFileSync(
  path.join(__dirname, '../clients/Vespa-Humla/REAL-TEMPLATES.json'),
  JSON.stringify(templates, null, 2)
);

console.log('\n💾 Saved to REAL-TEMPLATES.json');
console.log(`Total templates: ${withReplies.length}`);
